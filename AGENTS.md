...
10. **Package install cooldown** - `bunfig.toml` sets `minimumReleaseAge = 604800` (7 days) to block freshly-published npm versions, the vector behind recent supply-chain attacks (Shai-Hulud, `chalk`/`debug` worms). Don't lower it without discussion. To pull an urgent fix before its cooldown elapses, either bump `minimumReleaseAgeExcludeCveLevel` or add the package to a `minimumReleaseAgeExcludes` array in `bunfig.toml` — don't remove the global setting. Dependabot mirrors this with a 7-day `cooldown` block; security updates bypass it automatically.
...
## DB migrations
- Local dev DB runs in Docker: `docker compose up -d postgres` at the repo root brings up a fresh local Postgres at `postgres://superset:superset@localhost:5432/superset`.
- Use drizzle to manage the migration. You can see the schema at packages/db/src/schema. Never run a migration yourself.
- Create migrations by editing schema files under `packages/db/src/schema/`, then ask the user to run `bunx drizzle-kit generate --name="<sample_name_snake_case>"` from `packages/db/`.
